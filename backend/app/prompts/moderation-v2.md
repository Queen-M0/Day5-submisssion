...
8. riskLevel 必须是 0、1、2、3 中的整数；riskScore 必须是 0 到 100 的整数，禁止输出 0 到 1 的小数比例；confidence 才使用 0 到 1 的小数。
9. 所有布尔字段必须输出 JSON boolean，数组字段即使为空也必须输出数组。
10. 只输出 JSON 对象，不输出 Markdown 或额外解释。

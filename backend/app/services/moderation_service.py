...
def _moderation_prompt_version(provider: ModerationProvider) -> str:
    return getattr(provider, "moderation_prompt_version", getattr(provider, "prompt_version", "unknown"))


def _model_version(provider: ModerationProvider) -> str:
    return getattr(provider, "model_version", "unknown")
...
class ModerationService:
    ...
    def review(self, db: Session, content: Content) -> ModerationResult:
        ...
        except Exception as exc:  # noqa: BLE001 - route provider failures to manual review
        ...
        if self.secondary_provider is not None and context is not None and failure_reason is None:
            ...
            except Exception as exc:  # noqa: BLE001 - secondary model failure is a divergence signal
        ...
        record = ModerationRecord(
            ...
            prompt_version=_moderation_prompt_version(self.provider),
            ...
            or (
                ["当前内容", "所属话题"]
                + (["被回复楼层"] if content.parent_id else [])
                + ["最近 5 楼", "作者历史", "目标用户历史"]
            ),
            uncertainties=(result.uncertainties or [])
            + ([] if evidence_valid else ["证据原文无法在输入中定位"])
            + dual_uncertainties
        ...
        if self.secondary_provider is not None and context is not None and (secondary_result is not None or secondary_failure_reason):
            comparison = ModerationComparison(
                ...
                secondary_model_version=_model_version(self.secondary_provider),
                secondary_prompt_version=_moderation_prompt_version(self.secondary_provider),
            ...
            db.add(
                AuditLog(
                    ...
                    detail={
                        "primaryModel": _model_version(self.provider),
                        "secondaryModel": _model_version(self.secondary_provider),
                        "divergent": bool(divergence_reasons),

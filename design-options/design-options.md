...
| 对接输入 | `前后端对接与后端开发契约.md` |
| 参考格式 | `design-options(2).md` |
| 产品形态 | 单社区、多话题、线性楼层式文字讨论社区 |
| 团队规模 | 2 人 |
| 项目周期 | Day 5–Day 6 开发，Day 7 答辩 |
| 当前版本 | v1.0 |
| 当前状态 | 前端 Demo 已完成，后端与真实数据链路开发中 |
| 最后更新 | 2026-07-16 |

> **设计目标**：在“两天内可交付、可运行、可演示、可追溯”的约束下，为 PRD 中的 P0 能力提供可直接开发和联调的技术设计，同时为 P1/P2 保留清晰扩展边界。
...
当前 React + TypeScript 前端 Demo 已具备：
...
当前这些状态主要保存在前端本地 Demo 数据中。后端下一阶段的任务不是重新定义产品，而是将前端本地状态替换为数据库与真实 API 状态。
...
1. 缺少 `topics` 表，无法完整表达“社区 → 话题 → 楼层”；
2. `contents` 缺少 `topic_id`、`floor_number` 和明确目标用户；
3. 申诉缺少补充上下文；
4. 缺少申诉反证 AI 的结构化结果；
5. AI 建议和系统实际分流未完全分开；
6. 审核记录缺少意图、对象、上下文标签和不确定点；
7. 缺少“我的发布”真实接口；
8. 缺少完整时间线与审核历史接口；
9. 人工决定枚举与前端尚未完全统一；
10. Seed 身份与前端演示身份需要统一。
...
| C-02 | 前端 Demo 已完成 | 后端优先适配既有页面和数据契约，不重做 UI |
...
| C-15 | 当前规模较小 | 不引入 Kafka、RabbitMQ、Redis、向量库和独立工作流服务 |
...
| 前后端字段变化 | 以对接契约和 Pydantic Schema 为唯一基准 |
...
2. 当前前端 Demo 已经按照该产品形态实现；
3. 后端可以在现有表和接口基础上渐进补齐；
...
当前 Demo 状态位于 `DemoContext`，正式联调后采用：
...
| R-02 | 后端赶不上已完成前端 | 中 | 高 | 页面长期使用 DemoContext | 按三条主路径优先接 API |
...
| R-13 | UI 过度打磨 | 中 | 中 | 后端主链仍未完成 | 当前 Demo 够用，不再大改 |
...
> **设计状态：PASS，可进入 tasks 拆解、TDD 和真实后端联调阶段。**
